...
interface Props {
  market: Market;
  onRefresh?: () => void;
  /** Grid position index — drives stagger entrance delay (0-based) */
  staggerIndex?: number;
}
...
export function MarketCard({ market, onRefresh, staggerIndex = 0 }: Props) {
  ...
  const [tilt, setTilt] = useState({ rx: 0, ry: 0, lifted: false });
  ...
  function handleMouseMove(e: React.MouseEvent<HTMLDivElement>) {
    ...
    setTilt({ rx: dy * -5, ry: dx * 5, lifted: true });
  }

  function handleMouseLeave() {
    setTilt({ rx: 0, ry: 0, lifted: false });
  }
  ...
  const entranceDelay = `${Math.min(staggerIndex, 11) * 0.08}s`;
    ...
    <>
      ...
      >
        ...
        <div
          ...
          style={{
            transform: `perspective(1100px) rotateX(${tilt.rx}deg) rotateY(${tilt.ry}deg) translateY(${tilt.lifted ? "-3px" : "0"})`,
            transition: tilt.lifted
              ? "transform 0.12s ease"
              : "transform 0.55s cubic-bezier(0.22, 1, 0.36, 1)",
          }}
          className={`card flex flex-col gap-0 overflow-hidden
            ${isActive ? "card-active-aura" : ""}`}
        >
                      ...
                      <div
                        className={`progress-bar ${
                          isWinner
                            ...
                            : "bg-gradient-to-r from-arc-700 to-arc-400"
                        }`}
                        style={{ width: `${pct}%` }}
                      />
                    </div>
