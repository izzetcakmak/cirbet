...
import {
  LayoutDashboard, Home, Loader2, CheckCircle2, AlertCircle, Trophy,
} from "lucide-react";
...
function useUserBets(address: `0x${string}` | undefined) {
  ...
  const betsWithMarkets: BetWithMarket[] = useMemo(() => {
    ...
    marketResults.forEach((mr, i) => {
      ...
      bets.forEach((bet) => {
        if (bet.amount > 0n) result.push({ market, bet });
      });
    });
    return result;
  }, [marketResults, betResults]);

  function refetch() { refetchMarkets(); refetchBets(); }
...
export default function DashboardPage() {
  ...
  const pendingWin = betsWithMarkets.reduce((acc, { market, bet }) => {
    ...
    const gross = (bet.amount * totalPool) / optPool;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 space-y-8">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            { label: t("accountTotalBets"),   value: betsWithMarkets.length,           color: "text-arc-400" },
          ].map((s) => (
        ...
        <div className="flex gap-2 border-b border-border">
          {([
            ...
            { key: "claims",  label: t("accountClaims"),   count: betsWithMarkets.filter(({market, bet}) => market.state === 2 && market.winningOption === bet.optionIndex && !bet.claimed).length },
            { key: "refunds", label: t("accountRefunds"),  count: betsWithMarkets.filter(({market, bet}) => market.state === 2 && market.winningOption !== bet.optionIndex && !bet.claimed).length },
            ...
            >
              {tab_item.label}
              <span className="ml-2 text-xs bg-surface-2 rounded-full px-2 py-0.5">{tab_item.count}</span>
            </button>
          ))}
        </div>
              ...
              return (
                <div key={idx}
                  className={`bg-surface-1 border rounded-xl p-4 space-y-3 ${
                    isWinner ? "border-green-600/40" : isLoser ? "border-red-600/20" : "border-border"
                  }`}>
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex flex-wrap items-center gap-x-4 gap-y-1 mt-1.5 text-xs text-gray-500">
                        ...
                        {market.state === 2 && isWinner && !bet.claimed && (
                  ...
                  <div className="flex items-center gap-2">
                    <span className={`text-xs px-2 py-0.5 rounded-full border ${
                      market.state === 0 ? "text-arc-400 bg-arc-600/10 border-arc-600/30" :
                      market.state === 1 ? "text-yellow-400 bg-yellow-600/10 border-yellow-600/30" :
                      "text-green-400 bg-green-600/10 border-green-600/30"
                    }`}>
                      {["Active", "Locked", "Resolved"][market.state]}
                    </span>
